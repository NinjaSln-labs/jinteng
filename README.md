...
# 服务端 token：二进制/systemd 多为 ~/.lanvault/token 或 /root/.lanvault/token
# Docker：docker compose exec lanvault cat /data/token
export LANVAULT_TOKEN="$(tr -d '\n' < ~/.lanvault/token)"
lanvault list
lanvault run -e API_KEY=openai/key -- npm start
```

> 信任边界：默认 HTTP + Bearer，只适合**可信局域网**（或 VPN）。不要把 `8787` 裸暴露到公网。详见 [docs/security.md](docs/security.md)。
...
| `LANVAULT_PASSWORD` | master password（临时调试用；常驻服务请用 `_FILE`） |
