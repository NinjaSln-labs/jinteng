...
| 目标 | 做法 |
|------|------|
| 不明文落盘 | `jinteng.bin`：Argon2id 派生密钥 + AES-256-GCM |
| 不进 git | 仓库只保留 secret **名称**；忽略 `jinteng.bin` / `token` / `master.pass` / `.env` |
...
| 密钥 | 谁持有 | 用途 |
|------|--------|------|
| **Master password** | 仅服务端（或本机直连 CLI） | 解密 `jinteng.bin` |
...
- 备份 `jinteng.bin` 时必须同时安全保管 `master.pass`，否则无法恢复。
