...
| 不进 CI/agent 源码 | 用 `lanvault run` 运行时注入环境变量 |
...
| **API Token** (`lv_…`) | 服务端 + 受信任客户端 | 调用 `/v1/*` |

客户端 **不要** 分发 master password。Token 泄露后：在服务端 `lanvault token rotate`，更新各客户端。
...
- `lanvault get` / API GET 会取出明文——只用于调试，避免进日志、截图、聊天。
